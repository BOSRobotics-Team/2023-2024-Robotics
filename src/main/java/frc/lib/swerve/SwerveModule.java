...
public class SwerveModule {
  ...
  private void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
    if (isOpenLoop) {
      double percentOutput =
          desiredState.speedMetersPerSecond / RobotPreferences.Swerve.maxSpeed.get();
  ...
  private void setAngle(SwerveModuleState desiredState, boolean isForceAngle) {
    ...
    if (!isForceAngle
        && Math.abs(desiredState.speedMetersPerSecond)
            <= (RobotPreferences.Swerve.maxSpeed.get() * 0.01)) {
      angle = lastAngle;
    } else {
  ...
  public SwerveModuleState getState() {
    return new SwerveModuleState(
        inputs.driveVelocityMetersPerSec, Rotation2d.fromDegrees(inputs.anglePositionDeg));
  }

  public SwerveModulePosition getPosition() {
    return new SwerveModulePosition(
        inputs.driveDistanceMeters, Rotation2d.fromDegrees(inputs.anglePositionDeg));
  }
