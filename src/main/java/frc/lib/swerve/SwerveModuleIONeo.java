...
public class SwerveModuleIONeo implements SwerveModuleIO {
  ...
  private void configAngleMotor(int angleMotorID, String canBusID) {
    ...
    mAngleMotor.setIdleMode(
        DriveTrainConstants.angleNeutralMode == NeutralMode.Coast
  ...
  private void configDriveMotor(int driveMotorID, String canBusID) {
    ...
    mDriveMotor.setIdleMode(
        DriveTrainConstants.driveNeutralMode == NeutralMode.Coast
            ? CANSparkMax.IdleMode.kCoast
            : CANSparkMax.IdleMode.kBrake);
    ...
    // Multiply by the circumference to get meters per minute, divide by 60 to get meters per
    // second.
    driveEncoder.setVelocityConversionFactor(
        (1 / DriveTrainConstants.driveGearRatio) * DriveTrainConstants.wheelCircumference / 60.0);
